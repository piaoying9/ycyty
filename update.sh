...
update_version() {
    wget https://github.com/qd-today/qd/raw/"${remoteversion}"/requirements.txt -O /usr/src/app/requirements.txt && \
    ...
    if [ "$#" != 0 ];then
        if [ "$1" = "force" ];then
            ...
            then
                ...
                echo "For available version tags, see 'https://github.com/qd-today/qd/tags'"
            fi
        fi
    else
        # 默认更新至最新Tag版本
